...
/// Look up the full PlayerData key (`<steam id>_+_|<EOS account id>`) for each requested
/// player by scanning existing saves. The EOS suffix is generated by Epic's servers per
/// account and cannot be synthesized, so it must be read back from a save that already
/// contains it. Returns a map: pure steam id -> full key (only for players found in saves).
#[tauri::command]
pub async fn get_player_unique_ids(steam_ids: Vec<String>) -> AppResult<Value> {
    run_blocking(move || {
        let save_games_dir = get_save_games_dir()?;
        let requested: HashSet<String> = steam_ids.into_iter().collect();
        let mut map: HashMap<String, String> = HashMap::new();

        if !save_games_dir.exists() {
            return Ok(json!({ "success": true, "map": map }));
        }
        ...
        for entry in iter {
            ...
            if let Ok(save) = cli_handlers::parse_sav_file(path) {
                ...
                for id in ids {
                    // Strip suffix: online `<id>_+_|<eos id>`, offline `<id>-<15 chars>`
                    let pure: String = if let Some(idx) = id.find("_+_|") {
                        id[..idx].to_string()
                    ...
                    if requested.contains(&pure) && !map.contains_key(&pure) {
                        map.insert(pure, id);
                    }
                }
            }
