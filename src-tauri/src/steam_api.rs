...
impl SteamCacheManager {
    ...
    pub fn new() -> Result<Self, String> {
        ...
        let cache_dir = PathBuf::from(local_appdata).join("ETBSaveManager/steam_cache");
    ...
    fn load_cache(&mut self) -> Result<(), String> {
        ...
        println!("已加载Steam缓存，包含{}个条目，调用次数: {}", self.cache.len(), self.call_count);
        Ok(())
    }
    ...
    fn save_cache(&self) -> Result<(), String> {
        ...
        // 保存调用计数
        cache_data.insert("call_count".to_string(), serde_json::Value::Number(serde_json::Number::from(self.call_count)));
    ...
    pub fn cleanup_expired_cache(&mut self) -> usize {
        ...
        // 收集过期的缓存条目
        let expired_ids: Vec<String> = self.cache
            .iter()
            .filter(|(_, entry)| now - entry.last_updated > 2592000) // 30天
            .map(|(id, _)| id.clone())
            .collect();
    ...
    fn smart_cleanup(&mut self) {
        println!("执行智能清理，当前缓存条目数: {}", self.cache.len());
        ...
        println!("智能清理完成，删除了{}个条目，剩余{}个条目", 
                 self.cache.len() - self.cache.len(), self.cache.len());
    }
...
pub async fn decrypt_steam_api_key(encrypted_key: String) -> Result<String, String> {
    ...
    // 将Vec<u8>转换为String
    let api_key = String::from_utf8(decrypted).map_err(|e| format!("转换解密数据为字符串失败: {}", e))?;
...
pub async fn get_all_steam_cache_entries() -> Result<Vec<(String, String, u64, u32)>, String> {
    ...
    let result: Vec<(String, String, u64, u32)> = entries.into_iter()
...
pub async fn get_steam_usernames_command(
    ...
    // 获取加密存储的API密钥
    let local_appdata = std::env::var("LOCALAPPDATA").map_err(|e| e.to_string())?;
    let config_path = PathBuf::from(local_appdata).join("ETBSaveManager/config.json");
    ...
    let encrypted_api_key = config.get("steamApiKey")
        .and_then(|v| v.as_str())
...
pub async fn save_steam_api_key(api_key: String) -> Result<(), String> {
    ...
    if let Some(obj) = config.as_object_mut() {
        obj.insert("steamApiKey".to_string(), serde_json::Value::String(encrypted_api_key));
    }
