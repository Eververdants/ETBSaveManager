 ...
 * - 首屏入场：整体下滑淡入（.rise，延迟错开）
 * - 面包屑链接：下划线生长（.link-underline + .is-active）
 * - 滚动进度条：黄色辉光
 */
      ...
      <div className="border-b border-[var(--color-ink)]/15 dark:border-[var(--color-paper-3)]/15">
        <nav aria-label="Main" className="mx-auto flex h-14 max-w-[1400px] items-stretch justify-between px-4">
          ...
          <div className="flex items-stretch gap-0">
            <a
              href="#top"
              className="group relative flex items-center px-3 pr-5 font-mono text-[11px] uppercase tracking-[0.18em] text-[var(--color-ink)] transition-opacity hover:opacity-80 dark:text-[var(--color-paper)]"
            >
              {/* 黄色 tab 角标 */}
              <span aria-hidden="true" className="absolute left-0 top-0 h-full w-1.5 bg-[var(--color-accent)] transition-transform duration-300 group-hover:scale-y-90" />
          ...
          <div className="flex items-center gap-1.5">
            <a
              ...
              className="press inline-flex h-9 items-center gap-1.5 border border-transparent px-2.5 font-mono text-[11px] uppercase tracking-[0.16em] text-[var(--color-ink-2)] transition-colors hover:border-[var(--color-ink)] hover:text-[var(--color-ink)] dark:text-[var(--color-paper-3)] dark:hover:border-[var(--color-paper-3)] dark:hover:text-[var(--color-paper)]"
            >
