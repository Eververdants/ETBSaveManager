...
describe("detectDelimiter", () => {
  it("returns 'none' for empty input", () => {
    ...
    expect(detectDelimiter(null)).toBe("none");
    expect(detectDelimiter(undefined)).toBe("none");
  });
...
describe("parseName", () => {
  ...
  it("returns empty result for non-string input", () => {
    expect(parseName(123).originalName).toBe(123);
    expect(parseName({}).highlights).toEqual([]);
  });
...
describe("parseNames", () => {
  it("returns empty array for non-array input", () => {
    ...
    expect(parseNames("string")).toEqual([]);
...
describe("parseMultiple", () => {
  it("returns empty result for null/undefined input", () => {
    expect(parseMultiple(null).records).toEqual([]);
    expect(parseMultiple(undefined).records).toEqual([]);
  });

  it("parses simple line-separated names", () => {
    const result = parseMultiple("主线_01\n支线1_02");
  ...
  it("detects and reports duplicate names", () => {
    const result = parseMultiple("save_01\nsave_01");
  ...
  it("generates error when no valid data found", () => {
    const result = parseMultiple("# only comments");
  ...
  it("always returns at least one result", () => {
    fc.assert(
      fc.property(fc.string(), (template) => {
        const result = expandRange(template);
...
describe("property-based: parseCSVLine invariants", () => {
  it("preserves field count regardless of content", () => {
    fc.assert(
      fc.property(
        fc.array(
          fc.string({ maxLength: 5 }).filter((s) => !s.includes(",") && !s.includes('"')),
