...
function isHeaderRow(fields) {
  ...
  const matchCount = fields.filter((f) =>
    allAliases.some((alias) => alias.toLowerCase() === f.toLowerCase())
  ).length;
  return matchCount >= 1;
}
...
export function parseName(name) {
  ...
  // 2. 识别层级关键词（橙色高亮）
  const sortedLevelKeywords = Object.keys(LEVEL_KEYWORDS).sort(
  ...
  // 3. 识别难度关键词（橙色高亮）
  const sortedDifficultyKeywords = Object.keys(DIFFICULTY_KEYWORDS).sort(
    (a, b) => b.length - a.length
  );
  ...
  // 4. 识别背包关键词
  const sortedBackpackKeywords = Object.keys(BACKPACK_KEYWORDS).sort(
    (a, b) => b.length - a.length
  );
  ...
  for (let i = firstDataLineIndex; i < lines.length; i++) {
    ...
    for (const expandedName of expandedNames) {
      ...
      const record = {
        ...
        actualDifficulty:
          cleanValue(fields[columnMapping.actualDifficulty]) || null,
