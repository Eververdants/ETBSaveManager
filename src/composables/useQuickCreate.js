...
export function useQuickCreate() {
  ...
  const createSingleArchive = async (archive, basicArchive) => {
    try {
      ...
      const saveData = {
        ...
        level: archive.finalLevel || 'Level0',
        ...
        main_ending: false, // 默认主线
        meg_unlocked: false // 默认未解锁
      }
  ...
  const batchCreateArchives = async () => {
    // 获取要创建的存档列表（选中的或全部）
    const archivesToCreate = state.selectedArchiveIds.size > 0
  ...
  const loadDraft = () => {
    ...
    try {
      ...
      console.log('[QuickCreate] 第一个存档的最终配置:', state.archives[0] ? {
