...
// Cache data
const releaseNotesCache: Record<string, typeof releaseNotesZhCN> = {
  "zh-CN": releaseNotesZhCN,
  "en-US": releaseNotesEnUS,
  "zh-TW": releaseNotesZhTW,
};

type ReleaseDateOptions = Intl.DateTimeFormatOptions;

interface ReleaseNotesReturn {
  currentRelease: ComputedRef<typeof releaseNotesZhCN | null>;
  latestRelease: ComputedRef<typeof releaseNotesZhCN | null>;
  totalCount: ComputedRef<number>;
  formatDate: (dateString: string, options?: ReleaseDateOptions) => string;
...
export function useReleaseNotes(): ReleaseNotesReturn {
  ...
  const totalCount = computed((): number => (currentRelease.value ? 1 : 0));

  // Format date
  const formatDate = (dateString: string, options: ReleaseDateOptions = {}): string => {
    if (!dateString) return "";
    const date = new Date(dateString);
    const defaultOptions: ReleaseDateOptions = {
      year: "numeric",
      month: "long",
      day: "numeric",
      ...options,
