...
router.beforeEach((to, from, next) => {
  // 预加载标记为preload的路由
  if (to.meta.preload) {
    const component = to.matched.find(record => record.meta.preload)?.components?.default;
  ...
  if (to.meta.priority && to.meta.priority <= 2) {
    ...
    if (matchedRoute && typeof matchedRoute.components?.default === 'function') {
