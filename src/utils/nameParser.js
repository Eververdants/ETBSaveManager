...
export const BACKPACK_KEYWORDS = {
  新手包: "starter",
  标准包: "standard",
  豪华包: "deluxe",
  至尊包: "ultimate",
...
export const INDEX_PATTERN = /[_-]?(\d{1,4})$/;

// Range expansion regex - matches {1-50} or [001-100] or {A-Z}
export const RANGE_PATTERN = /[{[](\d+|[A-Za-z])-(\d+|[A-Za-z])[}\]]/g;
 ...
 * @returns {boolean}
 */
export function isHeaderRow(fields) {
 ...
 * @returns {string[]}
 */
export function expandRange(template) {
  const results = [];
...
export function parseMultiple(input) {
  ...
  for (let i = firstDataLineIndex; i < lines.length; i++) {
    ...
    const fields = delimiter !== "none" ? parseCSVLine(trimmedLine, delimiterChar) : [trimmedLine];
...
export function parseNames(names) {
  ...
  return names.filter((name) => name && typeof name === "string" && name.trim()).map((name) => parseName(name.trim()));
}
