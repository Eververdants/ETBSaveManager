...
// 获取动画参数的函数
export const getAnimationParams = (animationType = 'default', performanceMode = 'auto', animationQuality = 'medium') => {
  const devicePerf = detectDevicePerformance()
  const isLowPerfMode = performanceMode === 'low'
  
  // 根据动画质量调整基础参数
  const qualityMultiplier = animationQuality === 'high' ? 1.2 : 
                           animationQuality === 'low' ? 0.7 : 
                           animationQuality === 'disabled' ? 0 : 1
